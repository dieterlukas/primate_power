...
Which variables to include in Figure 2?
...
# Distribution of intersexual dominance systems according to relaxed definition
# Occurrence of three types
table(combined$mostlyfdom)

# Determine how many different species there are for each type of intersexual dominance
species_femdom_summarytable<-as.data.frame((combined %>% group_by(mostlyfdom) %>% summarize(unique(corrected_species_id)) ))

nrow(species_femdom_summarytable[species_femdom_summarytable$mostlyfdom=="f",])
nrow(species_femdom_summarytable[species_femdom_summarytable$mostlyfdom=="m",])

speciesvariation_femdom_summarytable<-as.data.frame((combined %>% group_by(corrected_species_id) %>% summarize(unique(mostlyfdom)) ))
...
allcontinuouspredictors<-c("sexratio", "female_average_relatedness", "Synchrony", "r_seasonality_value", "male_skew", "env_harshness", "rainfall_unpredictability", "female_canine_height", "male_canine_height", "females","males","homerange_overlap","perc_aggression_mm","receptive_synchrony","body_mass","testes_mass")
...
alldiscretepredictors<-c("cooperative_breeder", "monopolization", "jointaggression_males", "jointaggression_females", "female_dispersal", "female_infanticide", "female_evictions", "sexbias_dispersal", "MatSysPMK", "SocOrgPMK")
